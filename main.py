...
logging.basicConfig(filename="logs/main.log", filemode="w", format="%(asctime)s %(name)s:%(levelname)s:%(message)s", datefmt="%d-%M-%Y %H:%M:%S", level=logging.DEBUG, encoding="utf-8")
...
def token_check(base_url, token):
    ...
    resp = response.json();
    logging.info(f'call function [token_check], token = {token}, resp = {resp}')
    return resp['live']
...
def check_tokens(email_host, email_pass, email_sender, email_sender_name, email_receivers):
    ...
    for llm_type in LLMType:
        ...
        logging.info(f'llm_type = {llm_type.name}, base_url = {base_url}, token = {token}')
        # 如果有任何一个为空，则跳过
